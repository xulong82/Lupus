...
# yaa locus
yaa_loc <- list(chrX = c(169969759, 170931299), chrY = c(90745845, 91644698))
yaa_loc <- list(chrX = c(161031299, 171031299), chrY = c(90745845, 91644698)) # 10Mbp
axTrack <- GenomeAxisTrack()
idxTrack <- IdeogramTrack(genome = "mm10", chromosome = "chrX")
refGenes <- UcscTrack(genome = "mm10", chromosome = "chrX", track = "refGene", 
  from = yaa_loc$chrX[1], to = yaa_loc$chrX[2], trackType = "GeneRegionTrack", 
  rstarts = "exonStarts", rends = "exonEnds", name = "Ensembl Genes", fill = "firebrick1", 
  gene = "name", symbol = "name2", transcript = "name", strand = "strand")
symbol(refGenes) %>% unique

pdf("WES/yaa.pdf", width = 12, height = 4)
plotTracks(list(idxTrack, axTrack, refGenes), from = yaa_loc$chrX[1], to = yaa_loc$chrX[2], transcriptAnnotation = "symbol", stackHeight = 0.2)
dev.off()

# variants and effects
sf_bxsb = "./WES/BXSB_variants_filtered.vcf.gz"
...
var <- read.delim("Yaa/BXSB.vcf", comment = "#", stringsAsFactors = F)  # filter required
