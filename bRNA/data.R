...
setwd("~/Dropbox/GitHub/Lupus")
save(gene_expression_tpm, file = "bRNA/gene_expression_tpm.rdt")
save(gene_expression_cnt, file = "bRNA/gene_expression_cnt.rdt")
save(gene_expression_tpm, file = "bRNA/gene_expression_tpm_b6.rdt")
save(gene_expression_cnt, file = "bRNA/gene_expression_cnt_b6.rdt")

list.files("bRNA")
sample  <- read.csv("bRNA/trim.sh.o622832", stringsAsFactors = F, header = F)
sample <- gsub("_GES.*", "", sample$V1[2:8])

trim <- read.csv("bRNA/trim.sh.e622832", stringsAsFactors = F, header = F)
trim <- trim$V1[grep("^Input", trim$V1)]
trim <- gsub(".*\\(", "", gsub("%.*","", trim)) %>% as.numeric

files <- list.files("bRNA/log_rsem_b6", pattern = ".sh.e")
align_b6 <- lapply(files, function(x) read.csv(paste0("bRNA/log_rsem_b6/", x), stringsAsFactors = F, header = F))
align_b6 <- lapply(align_b6, function(x) x$V1[grep("at least one", x$V1)])
align_b6 <- sapply(align_b6, function(x) gsub(".*\\((.*)%.*", "\\1", x)) %>% as.numeric

files <- list.files("bRNA/log_rsem_pseudo_bxsb", pattern = ".sh.e")
align_bxsb <- lapply(files, function(x) read.csv(paste0("bRNA/log_rsem_pseudo_bxsb/", x), stringsAsFactors = F, header = F))
...
pdf("bRNA/pipeline.pdf", width = 8, height = 5)
